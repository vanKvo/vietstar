			...
			<div class="span10">
				...
				<form action="incoming.php" method="post">
					<input type="hidden" name="pmt_method"
						value="<?php echo htmlspecialchars($_GET['pmt_method'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" />
					<input type="hidden" name="invoice"
						value="<?php echo htmlspecialchars($_GET['invoice'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" />
					<select name="product_id" style="width:650px;" class="chzn-select" required>
						...
						for ($i = 0; $row = $result->fetch(); $i++) {
							...
							<option value="<?php echo $row['product_id']; ?>"><?php echo $row['product_code']; ?> |
								<?php echo $row['product_name']; ?> | Quantity Onhand: <?php echo $row['qty_onhand']; ?>
							</option>
							<?php
						}
						?>
					</select>
					<input type="number" name="qty_picked" value="1" min="1" placeholder="Qty" autocomplete="off"
